    ...
    <Link href={`/companies/${company.id}`}>
      <div className="bg-surface border border-border rounded-lg p-5 hover:shadow-md hover:border-primary/30 transition-all cursor-pointer group">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          <div>
            <p className="text-lg font-semibold text-foreground">{company.activeApplications}</p>
            <p className="text-xs text-text-muted">Active</p>
          </div>
          <div className="text-right">
            <p className="text-lg font-semibold text-foreground">{company.totalApplications}</p>
